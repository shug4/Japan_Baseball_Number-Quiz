...
const ALL_FILES_TEAM = [...CENTRAL_FILES, ...PACIFIC_FILES, 'umpires.json', '0mascots.json'];
...
function updateQCountBtns() {
  ...
  const allFiles = [
    ...
    'datasets/0mascots.json',
  ];
  const files = selectedTeamFiles.map(f => f.startsWith('datasets_ikusei/') ? f : `datasets/${f}`);
...
function handleAnswer(userVal) {
  ...
  const u = questions[currentIdx];
  const correct = userVal !== null && userVal === String(u.number);
...
document.getElementById('submit-btn').addEventListener('click', () => {
  const raw = toHalfWidth(document.getElementById('answer-input').value).trim();
  if (raw !== '') handleAnswer(raw);
});
...
document.getElementById('answer-input').addEventListener('input', () => {
  ...
  const inputDigits = raw.length;
  if (inputDigits === answerDigits) handleAnswer(raw);
});
